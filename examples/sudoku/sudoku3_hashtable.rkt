...
(define unitlist (py-add (py-add (for/py-list ([c (->sequence cols)])
                         ...
                         (for*/py-list ([rs (->sequence (make-py-tuple "ABC" "DEF" "GHI"))]
                           ...
                           (cross rs cs))))
(define units (for/hash ([s (->sequence squares)])
                (values s (for/py-list ([u (->sequence unitlist)]
                                        #:when (py-truth (py-in s u)))
                            u))))
(define peers (for/hash ([s (->sequence squares)])
                (values s (py-sub (py-set (:sum (py-get-index units s) (make-py-list)))
                                  (py-set (make-py-list s))))))


(define (parse_grid grid)
  (let/ec return
    (let ([vals (for/mutable-hash ([s (->sequence squares)])
                  (values s digits))])
